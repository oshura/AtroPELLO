...
| **14** | ✅ **Vuelo por ratón** (build 89) → plan `documentation/Plan_MiGo_Aracnidos.md` S0. `MouseFlightSystem` (flat+host, zona muerta 6 %, saturación 42 %, curva cuadrática) sobre canales analógicos nuevos de `Spaceship`; dispositivo Mi-Go en el outfit (`mouseFlight`/`turnTier`, opcionales); toggle tecla `c`; roll sigue en Q/E |
| **15** | ✅ **Mi-Go, Arácnidos y exterminio** (build 89) → `docs/RAZAS.md` §6b-§11c. `GateTuningState` estructurado y persistido; generador con `guaranteedInhabitedCount`; misión `exterminate` con cuota; **AracnidWarSystem** (telarañas destruibles + hostilidad al primer disparo + oleadas de cazas `externallyPiloted`); **Void Kinesis planetario** (`PlanetDrainBeam`); hostilidad por destrucción de planetas con caducidad de encargos; senda de Yig como gancho. Extracciones para la regla #1: paleta de planetas, escudo del cocoon, driver del outliner 2D, debris-colors → **GameEngine 11158→11155 (neto −3)** |
