...
export interface GiantPlanetOptions {
  /** If true, the provided radius is already the final world-space radius and should not be scaled. */
...
export class GiantPlanet extends Planet {
  constructor(id: string, colorName: PlanetColorName, radius: number, initialPos: Vector3, options?: GiantPlanetOptions) {
    // Multiply radius by 4 to make giants visually massive unless the snapshot already provides the final scale
    const multiplier = options?.radiusIsAbsolute ? 1 : 4;
