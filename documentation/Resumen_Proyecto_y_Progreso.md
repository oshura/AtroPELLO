...
- HUD y UI
  ...
  - Clima estratificado y temporal: `AtmosphereWeatherService` ahora mantiene cuatro capas (superficie, baja, media y superior), cada una con eventos independientes de ~2 min de duración, posibilidad de estado «calma» y probabilidades ajustadas (tormentas cerca del suelo, meteoros en la capa alta, polvo en enanas). El snapshot incluye `layerId/label` y los rangos de altitud activos para que HUD y gameplay sepan en qué estrato estás y anticipen el próximo cambio.
  - Precipitación, meteoros y rayos visibles: `ParticleEffectsService.updateWeatherPrecipitation()` amplía el spawn rate, tamaño y dispersión de lluvia/polvo, añade un tercer modo de meteoros con trazas incandescentes y mantiene partículas cerca de la cámara aunque vueles rápido. Los rayos ahora renderizan núcleo + aura mediante quads dobles y se generan con más segmentos/jitter, por lo que se ven nítidos entre cielo y suelo antes de que aparezca el flash global del `ScreenOverlayRenderer`.
  - Impulso post-aterrizaje: `GameEngine.captureShipKineticsSnapshot()` guarda `velocity/currentSpeed/targetSpeed/thrusterState` antes de teletransportar la nave y `restoreShipKineticsSnapshot()` los restituye al entrar en la atmósfera, respetando exactamente la velocidad que traías (sin clamp de 3u) y recalculando el vector forward si hacía falta. `applyAtmosphereLandingImpulse()` solo añade empuje cuando llegas casi detenido, así nunca pierdes la inercia que traías desde el vacío.
