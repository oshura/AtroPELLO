...
- HUD y UI
  ...
  - Despegue automático a 1000 u: tras un touchdown, `handleLandingTouchdown()` arma un flag atmosférico y `GameEngine.maybeTriggerAtmosphereAutoTakeoff()` vigila la altitud real (`computeAltitudeAboveGround()`). El CTA «Despegar» lanza primero `GroundTakeoffAnimation` (50 u / 5 u/s, cámara cockpit) que, además, invierte `setWingDeploymentProgress()` para que las alas se abran mientras dura la secuencia; más adelante, al superar 1000 u, la nave dispara automáticamente `startAtmosphereExitSequence()` para reproducir la fase orbital completa (audio, HUD, limpieza del renderer espacial y `exitAtmosphereScene()` al completarse).
  - Autolanding atmosférico coreografiado: `AtmosphereLandingAnimation` se extendió a 7 s (5 s de descenso + 2 s de giro previo al toque) y mantiene los 2 s de reposo posteriores. Los últimos 2 s rotan la nave 90° alrededor de la normal planetaria, atenúan el flare según el ángulo y usan `GameEngine.setWingDeploymentProgress()` para llevar las alas del 0 % al 100 %. Ahora las alas pivotan alrededor del eje nariz→thruster y se cruzan sobre el fuselaje como un ave justo antes del touchdown, reemplazando por completo el antiguo rig de anclaje y su SFX dedicado. `GameEngine.ensureAtmosphereLandingAirRushLoop()` mantiene `sfx_passby_air` de fondo, a los 2 s se dispara `Landing.wav` vía `playAtmosphereLandingApproachCue()` y, un segundo antes del touchdown, `spawnAtmosphereLandingDustSheets()` levanta capas de polvo volumétrico alrededor de la zona de contacto. La cámara permanece fija frente al punto de aterrizaje (CameraMode MANUAL) y sólo cede tras la cola de 2 s, momento en el que se reactivan colisiones e inputs y `openLandingPanelWithDelay()` respeta un mínimo de 7 s para abrir el panel. El GameEngine ahora preemite cualquier animación no crítica antes de lanzar la cinemática y deja registro cuando se vio obligado a usar la cámara heredada; únicamente las secuencias de landing/takeoff pueden bloquearla. Además, mientras la cinemática está activa se blinda la nave frente al drift y a las penalizaciones de turbulencia: no hay jitter, fuerzas laterales ni pérdida de aceleración hasta que termina la animación.
