...
- `GameInputHandler` consulta `KeyBindingsService`, normaliza la tecla pulsada y traduce la acción al valor por defecto esperado por el motor (por ahora la lógica central sigue escuchando las teclas legado como `w`, `m`, `l`, `i`, etc.).
...
5. **Motor del juego** – `GameEngine.handleKeyDown` contiene todo el comportamiento contextual. Algunos ejemplos relevantes:
   ...
   - `'l'` controla el grimorio y sincroniza la selección de hechizos.
...
- `DEFAULT_BINDINGS` sirve como tabla de verdad; `getDefaultKey(action)` siempre devuelve el valor legacy esperado por el motor sin importar el rebinding actual.
