...
`GameEngine.computeLandingThreat` activa la luz roja únicamente cuando detecta un game object hostil (marcado por `RelationService`) a ≤500u. El snapshot almacena el nombre/id del objetivo y la distancia redondeada para mostrarlo en `reasons`.

Si `threatActive` es true, `tryStartLandingSequence` rechaza la petición sin mostrar mensajes; el piloto rojo es el feedback.
